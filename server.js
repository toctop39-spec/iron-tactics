...
// Раздаем твой HTML файл (убедись, что он лежит в папке public и называется index.html)
...
io.on('connection', (socket) => {
    ...
    // --- НОВОЕ: Отправляем всем текущее количество игроков ---
    // io.engine.clientsCount — это встроенный счетчик подключений в Socket.io
    ...
    socket.on('disconnect', () => {
        ...
        // --- НОВОЕ: Обновляем счетчик онлайна при выходе ---
        io.emit('update_online', io.engine.clientsCount);
        // --------------------------------------------------
        ...
        for (const name in rooms) {
            ...
            if (room.players.includes(socket.id)) {
                ...
                delete rooms[name]; // Удаляем комнату, так как игра прервана
                console.log(`Room ${name} deleted because player left`);
                break; 
            }
        }
    });
